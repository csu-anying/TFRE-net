...
class block1(nn.Module):
    ...
    def __init__(self, filters):
        ...
        self.conv2d0 = nn.Sequential(
            nn.BatchNorm2d(1),
            nn.LeakyReLU(0.3),
            nn.Conv2d(1, filters // 2, kernel_size=(1, 5), padding=(0, 2))
        )
        self.SE1 = SE_Module(filters // 2, dim=2)

        self.conv2d0_1 = nn.Sequential(
            nn.BatchNorm2d(1),
            nn.LeakyReLU(0.3),
            nn.Conv2d(1, filters // 2, kernel_size=(5, 1), padding=(2, 0))
        )
        ...
        self.conv2d1 = nn.Sequential(
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters // 2, filters, kernel_size=(3, 11), padding=(1, 5))
        )

        self.conv2d1_1 = nn.Sequential(
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters // 2, filters, kernel_size=(11,3), padding=(5, 1))
        )

        self.conv2d2 = nn.Sequential(
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters, filters, kernel_size=(3, 17), padding=(1, 8))
        )
...
class block2(nn.Module):
    ...
    def __init__(self, filters):
        ...
        self.conv2d0 = nn.Sequential(
            ...
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters0, filters // 2, kernel_size=(3, 5), padding=(1, 2)),
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters // 2, filters // 2, kernel_size=(1, 11), padding=(0, 5))
        )

        self.conv2d0_1 = nn.Sequential(
            nn.BatchNorm2d(filters0),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters0, filters // 2, kernel_size=(5, 3), padding=(2, 1)),
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
        ...
        self.conv2d1 = nn.Sequential(
            ...
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters, filters, kernel_size=(1, 11), padding=(0, 5))
        )

        self.conv2d1_1 = nn.Sequential(
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters // 2, filters, kernel_size=(3, 1), padding=(1, 0)),
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters, filters, kernel_size=(13, 1), padding=(6, 0))
        )

        self.conv2d2 = nn.Sequential(
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters, filters, kernel_size=(1, 17), padding=(0, 8)),
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters, filters, kernel_size=(1, 21), padding=(0, (21-1)//2))
        )
    ...
    def forward(self, x):
        ...
        output2 = output1
        identity2 = output0
        identity2 = self.conv2d2_1(identity2)
        output2 = self.conv2d2(output2)
        output2 = self.SE1(output2)
        output2 = output1 + output2 + identity2

        output3 = output2
        identity3 = output0
        identity3 = self.conv2d2_1(identity3)
        output3 = self.conv2d2(output3)
        output3 = self.SE1(output3)
        output3 = output2 + output3 +identity3

        output3 = self.leakyrule(output3)
...
class block3(nn.Module):
    ...
    def __init__(self, filters,n):
        ...
        self.conv2d0 = nn.Sequential(
            nn.BatchNorm2d(filters0),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters0, filters // 2, kernel_size=(3, n), padding=(1, (n-1) // 2)),
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters // 2, filters // 2, kernel_size=(1, n+6), padding=(0, (n+6-1) // 2))
        )

        self.conv2d0_1 = nn.Sequential(
            nn.BatchNorm2d(filters0),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters0, filters // 2, kernel_size=(n-2, 3), padding=((n -2 - 1) // 2, 1)),
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters // 2, filters // 2, kernel_size=(n, 1), padding=((n - 1) // 2, 0))
        )
        ...
        self.conv2d1 = nn.Sequential(
            nn.BatchNorm2d(filters // 2),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters // 2, filters, kernel_size=(1, n+6), padding=(0, (n+6-1) // 2)),
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters, filters, kernel_size=(1, n+12), padding=(0, (n+12-1) // 2))
        )
        ...
        self.conv2d2 = nn.Sequential(
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Conv2d(filters, filters, kernel_size=(1, n+6), padding=(0, (n+6-1) // 2)),
            nn.BatchNorm2d(filters),
            nn.LeakyReLU(0.3),
            nn.Dropout(0.5),
            nn.Conv2d(filters, filters, kernel_size=(1, n+12), padding=(0, (n+12-1) // 2))
    ...
    def forward(self, x):
        ...
        identity3 = self.conv2d2_1(identity3)
        output3 = self.conv2d2(output3)
        identity3 = self.SE1(identity3)
        output3 = self.SE1(output3)
        output3 = output2 + output3 +identity3

        output4 = output3
        identity4 = output4
        identity4 = self.conv2d2_1(identity4)
        output4 = self.conv2d2(output4)
        identity4 = self.SE1(identity4)
        output4 = self.SE1(output4)
        output4 = output3 + output4 +identity4

        output5 = output4
        identity5 = output5
        identity5 = self.conv2d2_1(identity5)
        output5 = self.conv2d2(output5)
        identity5 = self.SE1(identity5)
        output5 = self.SE1(output5)
        output5 = output4 + output5 +identity5

        output5 = self.leakyrule(output5)
...
class SEnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.blk1 = block1(4)
        self.blk2 = block2(4)
        self.blk3_1 = block3(8,5)
        self.blk3_2 = block3(8, 7)
        # self.blk3_3 = block3(4, 15)
