...
class PDF3(nn.Module):
    ...
    def forward(self, batch, num_classes=7, label7=None, dataset='PTBXL', criter='CE', return_only_output=False):
        ...
        # >=26
        x = self.act(self.bn1(self.conv1(stacked_data, edge_index)))
        ...
        # x_fused = x * (gate_tr+gate_gat) + tr_tensor * (1 - gate_tr) + gat * (1-gate_gat)
        ...
        # 28 lose
        # final_logits[:, 6] = logit6[:, 1]  # label 6
        # final_logits[:, 1] = logit6[:, 0]*logitNum[:,0]*logit13[:, 0]  # label 1
        # final_logits[:, 3] = logit6[:, 0]*logitNum[:,0]*logit13[:, 1]  # label 3
        # final_logits[:, 0] = logit6[:, 0]*logitNum[:,1]*logit0245[:, 0]  # label 0
        # final_logits[:, 2] = logit6[:, 0]*logitNum[:,1]*logit0245[:, 1]  # label 2
        # final_logits[:, 4] = logit6[:, 0]*logitNum[:,1]*logit0245[:, 2]  # label 4
        # final_logits[:, 5] = logit6[:, 0]*logitNum[:,1]*logit0245[:, 3]  # label 5
        ...
        final_logits = 0.3 * final_logits + 0.7 * self.ensemble_head(MMfeature)  # 27 28
        # final_logits = 0.2 * final_logits + 0.8 * self.ensemble_head(x_flat)  # 24
        # final_logits = 0.5 * final_logits + 0.5 * self.ensemble_head(x_flat)  # 25 26
        # logits 路由合并 20
        ...
        if label7 is not None:
            if 'PTBXL' in dataset:
                ...
                # [1, 1, 1, 1, 1, 1, 1]).cuda()  # TB-net-new_14
                # [1, 1.5, 1, 1.25, 1, 1, 1]).cuda()  # TB-net-new_15
                # [1, 2, 1, 1, 1, 1, 1]).cuda()  # TB-net-new_16
                [1, 1.7, 1, 1.25, 1, 1, 1]).cuda()  # TB-net-new_17 19 20 21 22 26
