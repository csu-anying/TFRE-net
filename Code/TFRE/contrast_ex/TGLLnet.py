...
class Mymodel1(nn.Module):
    def __init__(self, layers_graph=[1, 2], num_adj=3, num_chan=62,
                 num_feature=5, hidden_graph=16, K=2, dropout=0.25, graph2token='Linear', encoder_type='GCN',
                 num_classes=10):
    ...
    def forward(self, x):
        ...
        # temporal contextual convolution
        # x = x1
    ...
    def get_adj(self, self_loop=True):
        ...
        if self_loop:
            adj = adj + torch.eye(num_nodes).to(self.adjs.device)
            # adj = adj * (1 - torch.eye(num_nodes).to(self.adjs.device)) + torch.eye(num_nodes).to(self.adjs.device)
        else:
            adj = adj * (1 - torch.eye(num_nodes).to(self.adjs.device))  # diagnal is set to 0
        return adj
...
def Mymodel(**kwargs):
    model = Mymodel1(layers_graph=[4], num_adj=1,
                     num_chan=12, num_feature=1, hidden_graph=4,
                     K=3, dropout=0.25, graph2token='Linear', encoder_type='Cheby', **kwargs)
    return model